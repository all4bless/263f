...
def time_step(q0, u, dt, ctime):
    ...
    u_new = (q - q0) / dt
    ctime += dt
    return q, u_new, ctime

def plot_current_state(q, ctime):
    x1 = q[::2]
    x2 = q[1::2]
    plt.figure(1)
    plt.clf()
    ...
    plt.plot(x1, x2, 'ko-')
    plt.title(f't={ctime:.6f}')
    plt.axis('equal')
    plt.xlabel('x [m]')
    plt.ylabel('y [m]')
    plt.show()

def compute_mid_angle(q, midNode):
    idx = 2 * midNode
    vec1 = np.array([q[idx - 2], q[idx - 1], 0]) - np.array([q[idx - 4], q[idx - 3], 0])
    vec2 = np.array([q[idx], q[idx + 1], 0]) - np.array([q[idx - 2], q[idx - 1], 0])
    angle = np.degrees(np.arctan2(np.linalg.norm(np.cross(vec1, vec2)), np.dot(vec1, vec2)))
    return angle

# 主循环
timeStep = 1
while timeStep < Nsteps:
    q, u, ctime = time_step(q0, u, dt, ctime)
    if q is None:
        break
    q0 = q

    if timeStep % plotStep == 0:
        plot_current_state(q, ctime)

    all_pos[timeStep] = q[2 * midNode - 1]
    all_v[timeStep] = u[2 * midNode - 1]
    midAngle[timeStep] = compute_mid_angle(q, midNode)

    timeStep += 1

# 绘制结果
def plot_results(t, data, xlabel, ylabel, filename, color='b'):
    plt.figure()
...
plot_results(t, all_pos, 'Time, t [s]', 'Displacement, $\\delta$ [m]', 'fallingBeam.png')
plot_results(t, all_v, 'Time, t [s]', 'Velocity, v [m/s]', 'fallingBeam_velocity.png')
plot_results(t, midAngle, 'Time, t [s]', 'Angle, $\\alpha$ [deg]', 'fallingBeam_angle.png', color='r')
